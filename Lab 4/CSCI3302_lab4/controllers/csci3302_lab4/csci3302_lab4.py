...
angles = np.linspace(-LIDAR_ANGLE_RANGE/2, LIDAR_ANGLE_RANGE/2, LIDAR_ANGLE_BINS)
...
while robot.step(SIM_TIMESTEP) != -1:     
    ...
    def obs_coords(i, sensor):
        rx = sensor*math.cos(angles[i])
        ry = sensor*math.sin(angles[i])
        
        
        x = rx * math.sin(pose_theta) - ry * math.cos(pose_theta) + pose_x
        y = rx * math.cos(pose_theta) + ry * math.sin(pose_theta) + pose_y
    ...
    i = 0
    x,y = disp_coords(pose_x, pose_y)
    
    ##### Part 4: Draw the obstacle and free space pixels on the map
    for sensor in lidar_sensor_readings:
        xl,yl = obs_coords(i, sensor)        
        if xl != -1 and yl != -1 and xl < 301 and xl >= 0 and yl < 301 and yl >= 0:
            
            display.setColor(0xFFFFFF)
            display.drawLine(x,y,xl,yl)
            
            display.setColor(0x0000FF)
            display.drawPixel(xl,yl)
